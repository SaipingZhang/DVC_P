...
parser.add_argument("--test_Seq", default='D')
parser.add_argument("--path", default='BasketballDrive-1920x1072')
...
tesult_path= './result/'+args.test_Seq+'/'+str(args.l)+'/'
os.makedirs(tesult_path, exist_ok=True)
files = open(r'./result/'+args.test_Seq+'/'+str(args.l)+'/'+args.path+'.txt', 'a+')
files.truncate()
...
path = args.path + '/'#e.g. BasketballPass
path_com = args.path + '_com_' + args.mode  + '_' + str(args.l) + '/'#BasketballPass_com_PSNR_1024
path_bin = args.path + '_bin_' + args.mode  + '_' + str(args.l) + '/'#BasketballPass_bin_PSNR_1024
...
F1 = misc.imread(path + 'f001.png')#read BasketballPass/f001.png
Height = np.size(F1, 0)
Width = np.size(F1, 1)
...
with tf.variable_scope("flow_motion"):
    ...
    flow_tensor, _, _, _, _, _ = motion.optical_flow(Y0_com, Y1_raw, batch_size, Height, Width)#生成光流即运动信息f(1→0)

# Encode flow
flow_latent = CNN_img_newStructure.MV_analysis(flow_tensor, args.N, args.M)
...
flow_hat = CNN_img_newStructure.MV_synthesis(flow_latent_hat, args.N)
...
res_latent = CNN_img_newStructure.Res_analysis(Res, num_filters=args.N, M=args.M)
...
Res_hat = CNN_img_newStructure.Res_synthesis(res_latent_hat, num_filters=args.N)

Y1_com = Res_hat + Y1_MC
Y1_com = tf.clip_by_value(Y1_com, 0, 1)

if args.metric == 'PSNR':
    train_mse = tf.reduce_mean(tf.squared_difference(Y1_com, Y1_raw))
    quality = 10.0*tf.log(1.0/train_mse)/tf.log(10.0)#PSNR
elif args.metric == 'MS-SSIM':
    quality = tf.math.reduce_mean(tf.image.ssim_multiscale(Y1_com, Y1_raw, max_val=1))#MS-SSIM

save_path = './OpenDVC_PSNR_' + str(args.l)
summary_writer = tf.summary.FileWriter(save_path, sess.graph)

saver = tf.train.Saver(max_to_keep=None)

model_path = 'E:/DVC/model.ckpt-700000'
...
for gop_number in range(np.int(np.ceil(args.frame/args.GOP))):#n
    #I frame
    f = gop_number * args.GOP#10n
    if args.mode == 'PSNR':
        os.system('D:/DVC/OpenDVC-master/bpg-0.9.8-win64/bpgenc -f 444 -m 9 ' + path + 'f' + str(f + 1).zfill(3) + '.png -o ' + path_bin + str(f + 1).zfill(3) + '.bin -q ' + str(I_QP))
        os.system('D:/DVC/OpenDVC-master/bpg-0.9.8-win64/bpgdec -o '+ path_com + 'f' + str(f + 1).zfill(3) + '.png '+ path_bin + str(f + 1).zfill(3) + '.bin')
    ...
    bits = os.path.getsize(path_bin + 'f' + str(f + 1).zfill(3) + '.bin')
    bits = bits * 8
    ...
    F0_com_test = np.expand_dims(F0_com_test, axis=0)
    F0_raw = np.expand_dims(F0_raw, axis=0)

    if args.metric == 'PSNR':
        mse = np.mean(np.power(np.subtract(F0_com / 255.0, F0_raw / 255.0), 2.0))
        quality_frame[f] = 10 * np.log10(1.0 / mse)#PSNR
    elif args.metric == 'MS-SSIM':
        quality_frame[f] = MultiScaleSSIM(F0_com, F0_raw, max_val=255)#MS-SSIM

    bits_frame[f] = bits
    print('Frame', f + 1, args.metric + ' =', quality_frame[f], 'bpp =', bits_frame[f] / Height / Width,file=files)
...
summary_str = sess.run(merged_summary_op, feed_dict={Y0_com: F0_com_test / 255.0,
                                                     Y1_raw: F1_raw / 255.0})
summary_writer.add_summary(summary_str, 0)

print('Average ' + args.metric + ' =', quality_ave, 'Average bpp =', bits_ave,file=files)
